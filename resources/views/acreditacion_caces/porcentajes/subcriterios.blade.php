            ...
            @if (!$loop->last)
                <div class="card">
                    <div class="card-header pb-2">
                        <h6 class="fw-normal text-pacifico text-uppercase">{{ $criterio->criterio }}</h6>
                    </div>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($criterio->subcriterios as $subcriterio)
                                        <tr>
                                            ...
                                            <td><input
                                                    class="form-control form-control-sm {{ empty($subcriterio->porcentaje) ? 'alert-danger' : '' }} {{ $subcriterio->porcentaje != round($criterio->porcentaje / $totalSubcriterios, 3) ? 'text-danger' : '' }}"
                                                    type="number" min=0 max=100 step="0.001"
                                                    name="{{ $subcriterio->id }}[porcentaje]"
                                                    value="{{ round($criterio->porcentaje / $totalSubcriterios, 3) }}">
