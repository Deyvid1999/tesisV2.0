<div>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle text-uppercase pt-2 pb-2">
                        ...
                        <tbody>
                            ...
                            @else
                                @foreach ($arch_docentes as $arch_docente)
                                    <tr>
                                        <td>
                                            {{ $loop->iteration }}
                                        </td>
                                        <td>
                                            {{ $arch_docente->observacion }}
                                        </td>                                        
                                        <td>
                                            @isset($arch_docente)
                                                @if ($arch_docente->archivo != '')
                                                    <a class="text-actualizar" title="Descargar" target="_blank"
                                                        href="{{ asset('storage') . '/' . $arch_docente->archivo }}"><i class="fas fa-eye"></i> Ver
                                                    </a>
                                                @else
    ...
    {{-- MODALES --}}
    @include('livewire.criterio2.fuente10.modal')
</div>
