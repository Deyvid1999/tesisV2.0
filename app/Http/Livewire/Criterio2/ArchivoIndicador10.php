...
class ArchivoIndicador10 extends Component
{
    use WithFileUploads;
    ...
    public function mount($id_indicador, $id_evaluacion)
    {
        ...
        foreach ($this->fuentesInformaciones as $fuente) {
            $arch_docente = ArchDocente::where('fuente_informacion_id', $fuente->id)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->first();
            $this->archivo_url[$fuente->id] = isset($arch_docente->archivo) ? $arch_docente->archivo : null;
            $this->archivo[$fuente->id] = null;
            $this->observacion[$fuente->id] = isset($arch_docente->observacion) ? $arch_docente->observacion : null;
        }
    }

    public function render()
    {
        return view('livewire.criterio2.archivo-indicador10.view');
    }

    public function guardarArchivoIndicador10()
    {
        ...
        foreach ($this->archivo as $key => $archivo) {
            if ($key != 0) {                
                if ($archivo == null) {
                    $arch_docente = ArchDocente::where('fuente_informacion_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->first();
                    if(isset($arch_docente->archivo)){
                        $nombre_archivo = $arch_docente->archivo;
                    } else{
                        $nombre_archivo = null;
                    }
                } else {
        ...
        foreach ($datos as $key => $item) {
            $id = ArchDocente::where('fuente_informacion_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->get()->first();
            if ($id != null) {
                //update
                ArchDocente::where('fuente_informacion_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->update($item);
            } else {
                //insert
                ArchDocente::insert($item);
            }
        }
