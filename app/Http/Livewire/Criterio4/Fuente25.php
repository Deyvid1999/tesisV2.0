...
class Fuente25 extends Component
{
    use WithFileUploads;
    ...
    {
        $arch_investigaciones = ArchInvestigacion::where('fuente_informacion_id', $this->fuente_id)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->get();
        return view('livewire.criterio4.fuente25.view', compact('arch_investigaciones'));
    }
    ...
    {
        $dato = ArchInvestigacion::findOrFail($id);
    ...
    public function update()
    {
        if ($this->id_arch_programa) {
            ...
            $dato = ArchInvestigacion::find($this->id_arch_programa);
            $dato->update([
                'archivo' => $nombre_archivo,
                'observacion' => $this->observacion,
                'user_id' => auth()->user()->id,
    ...
    {
        ArchInvestigacion::find($this->delete_id)->delete();
        $this->dispatchBrowserEvent('close-modal');
