...
class Indicador24 extends Component
{
    ...
    public function mount($id_indicador, $id_evaluacion)
    {
        ...
        foreach ($this->elementosFundamentales as $elemento) {
            $res_investigacion = ResInvestigacion::where('elemento_fundamental_id', $elemento->id)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->first();
            $this->porcentaje[$elemento->id] = isset($res_investigacion->porcentaje) ? $res_investigacion->porcentaje : '0.00';
            $this->valoracion[$elemento->id] = isset($res_investigacion->valoracion) ? $res_investigacion->valoracion : null;
            $this->observacion[$elemento->id] = isset($res_investigacion->observacion) ? $res_investigacion->observacion : null;
        }
    }
    ...
    {
        return view('livewire.criterio4.indicador24.view');
    }

    public function guardarIndicador24()
    {
        $datos = [];
        foreach ($this->valoracion as $key => $value) {
            if ($key != 0) {
        ...
        foreach ($datos as $key => $item) {
            $id = ResInvestigacion::where('elemento_fundamental_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->get()->first();
            if ($id != null) {
                //update
                ResInvestigacion::where('elemento_fundamental_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->update($item);
            } else {
                //insert
                ResInvestigacion::insert($item);
            }
        }
