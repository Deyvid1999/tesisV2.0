...
class ArchivoIndicador21 extends Component
{
    use WithFileUploads;
    ...
    public function mount($id_indicador, $id_evaluacion)
    {
        ...
        foreach ($this->fuentesInformaciones as $fuente) {
            $arch_academico = ArchAcademico::where('fuente_informacion_id', $fuente->id)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->first();
            $this->archivo_url[$fuente->id] = isset($arch_academico->archivo) ? $arch_academico->archivo : null;
            $this->archivo[$fuente->id] = null;
            $this->observacion[$fuente->id] = isset($arch_academico->observacion) ? $arch_academico->observacion : null;
        }
    }

    public function render()
    {
        return view('livewire.criterio3.archivo-indicador21.view');
    }

    public function guardarArchivoIndicador21()
    {
        ...
        foreach ($this->archivo as $key => $archivo) {
            if ($key != 0) {                
                if ($archivo == null) {
                    $arch_academico = ArchAcademico::where('fuente_informacion_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->first();
                    if(isset($arch_academico->archivo)){
                        $nombre_archivo = $arch_academico->archivo;
                    } else{
                        $nombre_archivo = null;
                    }
                } else {
        ...
        foreach ($datos as $key => $item) {
            $id = ArchAcademico::where('fuente_informacion_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->get()->first();
            if ($id != null) {
                //update
                ArchAcademico::where('fuente_informacion_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->update($item);
            } else {
                //insert
                ArchAcademico::insert($item);
            }
        }
