...
class Indicador14 extends Component
{
    ...
    public function mount($id_indicador, $id_evaluacion)
    {
        ...
        foreach ($this->elementosFundamentales as $elemento) {
            $res_condicion_institucion = ResAcademico::where('elemento_fundamental_id', $elemento->id)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->first();
    ...
    {
        return view('livewire.criterio3.indicador14.view');
    }

    public function guardarIndicador14()
    {
        $datos = [];
        foreach ($this->valoracion as $key => $value) {
            if ($key != 0) {
        ...
        foreach ($datos as $key => $item) {
            $id = ResAcademico::where('elemento_fundamental_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->get()->first();
            if ($id != null) {
                //update
                ResAcademico::where('elemento_fundamental_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->update($item);
            } else {
                //insert
                ResAcademico::insert($item);
            }
        }
