...
class Fuente15 extends Component
{
    use WithFileUploads;
    ...
    {
        $arch_academicos = ArchAcademico::where('fuente_informacion_id', $this->fuente_id)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->get();
        return view('livewire.criterio3.fuente15.view', compact('arch_academicos'));
    }
    ...
    {
        $dato = ArchAcademico::findOrFail($id);
    ...
    public function update()
    {
        if ($this->id_arch_programa) {
            ...
            $dato = ArchAcademico::find($this->id_arch_programa);
            $dato->update([
                'archivo' => $nombre_archivo,
                'observacion' => $this->observacion,
                'user_id' => auth()->user()->id,
    ...
    {
        ArchAcademico::find($this->delete_id)->delete();
        $this->dispatchBrowserEvent('close-modal');
