...
class Indicador28 extends Component
{
    ...
    public function mount($id_indicador, $id_evaluacion)
    {
        ...
        foreach ($this->elementosFundamentales as $elemento) {
            $res_vinculacion = ResVinculacion::where('elemento_fundamental_id', $elemento->id)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->first();
            $this->porcentaje[$elemento->id] = isset($res_vinculacion->porcentaje) ? $res_vinculacion->porcentaje : '0.00';
            $this->valoracion[$elemento->id] = isset($res_vinculacion->valoracion) ? $res_vinculacion->valoracion : null;
            $this->observacion[$elemento->id] = isset($res_vinculacion->observacion) ? $res_vinculacion->observacion : null;
        }
    }
    ...
    {
        return view('livewire.criterio5.indicador28.view');
    }

    public function guardarIndicador28()
    {
        $datos = [];
        foreach ($this->valoracion as $key => $value) {
            if ($key != 0) {
        ...
        foreach ($datos as $key => $item) {
            $id = ResVinculacion::where('elemento_fundamental_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->get()->first();
            if ($id != null) {
                //update
                ResVinculacion::where('elemento_fundamental_id', $key)->where('evaluacion_id', $this->evaluacion_id)->where('indicador_id', $this->indicador_id)->update($item);
            } else {
                //insert
